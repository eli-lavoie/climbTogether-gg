...
const DetailsEdit = props => {
  ...
  const editListing = () => {
    ...
    LocalDataManager.update("LFGPosts", newListingObj, post.id)
    setAlertMsg("Your post has successfully been updated.")
  ...
  const getPostData = () => {
    ...
    .then(postData => {
      ...
      setMinRankId(postData.reqRankMin)
      setMaxRankId(postData.reqRankMax)
      setRoleId(postData.reqRole)
      setGameId(postData.gameType)

    })
  }
  const getMinRankName = () => {
    LocalDataManager.get("ranks", minRankId)
    .then(result => {
      const minTier = result.tier
      const minDivision = result.division
      if(minRankId >= 25){
        setMinRankString(minTier)
      }
      else{
        const minRankName = minTier + " " + minDivision
        setMinRankString(minRankName)
      }
    })
  }
  
  const getMaxRankName = () => {
    LocalDataManager.get("ranks", maxRankId)
    .then(result => {
      const maxTier = result.tier
      const maxDivision = result.division
      if(maxRankId >= 25){
        setMaxRankString(maxTier)
      }
      else{
        const maxRankName = maxTier + " " + maxDivision
        setMaxRankString(maxRankName)
      }
    })
  }

  const getGameMode = () => {
    LocalDataManager.get("gametypes", gameId)
    .then(result => {setGameMode(result.type)})
  }
  ...
  return(
    <>
      <Container>
        ...
        <Button className="submit-button" onClick={() => editListing()}>Submit Post</Button>
...
export default withRouter(DetailsEdit)